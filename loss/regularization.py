...
class Perturbation:
    ...
    """
    @classmethod
    def _add_noise_to_tensor(cls, tens: torch.Tensor, over_dim: int = 0, perturbation_strength: float = 1.) -> torch.Tensor:
        """
        ...
        return tens + torch.randn_like(tens) * tens.std(dim=over_dim) * perturbation_strength
        # return tens + torch.randn_like(tens)

    @classmethod
    def perturb_tensor(cls, tens: torch.Tensor, n_samples: int, perturbation: bool = True, perturbation_strength: float = 1.) -> torch.Tensor:
        """
        ...
        if perturbation:
            tens = cls._add_noise_to_tensor(tens, perturbation_strength=perturbation_strength)
