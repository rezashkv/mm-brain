...
from .functional import reglu, geglu

ERROR_MESSAGE = 'An unexpected error occurred. Please contact authors.'
    ...
    def make_baseline(
        ...
        * the dropout rate is the same for all dropout layers
        ...
        Args:
            ...
            activation: type of nonlinearity after each layer
            batch_norm: whether to add batch normalization after each layer
        Returns:
            MLP
    ...
    def make_baseline(
        ...
        * :code:`Norm` = :code:`BatchNorm1d`
        ...
        Args:
            ...
            d_out: the output size of the network.
...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        for tensor in [q, k, v]:
            assert tensor.shape[-1] % self.n_heads == 0, ERROR_MESSAGE
...
class Transformer(nn.Module):
    ...
    def _start_residual(self, layer, stage, x):
        assert stage in ['attention', 'ffn'], ERROR_MESSAGE
        x_residual = x
...
class FTTransformer(nn.Module):
    ...
    References:
        * [gorishniy2021revisiting] Yury Gorishniy, Ivan Rubachev, Valentin Khrulkov, Artem Babenko,
         "Revisiting Deep Learning Models for Tabular Data", 2021
        * [vaswani2017attention] Ashish Vaswani, Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones,
         Aidan N. Gomez, Lukasz Kaiser, Illia Polosukhin, "Attention Is All You Need", 2017
    """
    ...
    def __init__(
        ...
        self.d_out = self.transformer.d_out
...
class StudentG(nn.Module):
    def __init__(self, args, input_dim, hidden_dim):
        super(StudentG, self).__init__()
        self.args = args
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim

        self.non_linearity = torch.relu
        self.dropout = nn.Dropout(self.args.student_G_dropout)
        ...
        self.dims = [self.input_dim, 512, self.hidden_dim, self.hidden_dim]

    def forward(self, x_in, dropout=True):
        if x_in.shape[1] != self.input_dim:
            raise ValueError('input length is not equal to {}'.format(self.layers_dims[0]))
...
class StudentG2(nn.Module):  # August 18
    def __init__(self, args, input_dim, hidden_dim):
        super(StudentG2, self).__init__()
        self.args = args
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.layers_dim = self.calculate_dims()
        self.dims = [self.input_dim] + self.layers_dim
        self.blocks = nn.ModuleList()

        if self.args.student_G_act == 'relu':
            self.activation_fn = nn.ReLU()
        elif self.args.student_G_act == 'leaky_relu':
        ...
        for i in range(1, len(self.layers_dim)):
            if i <= 2:
                p_drop = self.args.student_G_dropout
            else:
                p_drop = 0.
            self.blocks.append(GANBlock(self.args, self.layers_dim[i - 1], self.layers_dim[i],
                                        activation_fn=self.activation_fn, p_drop=p_drop))
        p_drop = self.args.student_G_dropout
        self.blocks.append(GANBlock(self.args, self.layers_dim[-1], self.hidden_dim,
                                        activation_fn=self.activation_fn, p_drop=p_drop))
...
class GANBlock(nn.Module):
    def __init__(self, args, dim_in, dim_out, activation_fn, p_drop):
        super(GANBlock, self).__init__()
        self.args = args
        self.dim_in = dim_in
        self.dim_out = dim_out
        self.activation_fn = activation_fn
        self.layers, self.shortcut = nn.ModuleList(), nn.ModuleList()
        self.dropout = nn.Dropout(p_drop)
        self.build_block()

    def build_block(self):
        # Layers: BN -> A -> FC -> BN -> A -> FC
        self.layers.append(nn.BatchNorm1d(self.dim_in))
        self.layers.append(self.activation_fn)
        self.layers.append(nn.Linear(self.dim_in, self.dim_out))
        self.layers.append(nn.BatchNorm1d(self.dim_out))
        self.layers.append(self.activation_fn)
        self.layers.append(nn.Linear(self.dim_out, self.dim_out))

        # Shortcut
        self.shortcut.append(nn.Linear(self.dim_in, self.dim_out))
